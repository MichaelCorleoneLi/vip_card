# Created by John Jiang at 2018/3/16 14:25
...
class Hashids(object):
    ...
    def __init__(self, salt='', min_length=0, alphabet=ALPHABET, app=None):
        """
    ...
    def init_app(self, app):
        ...
        self._min_length = max(int(app.config.get('HASHIDS_MIN_LENGTH', self._min_length)), 0)
        self._salt = salt = app.config['SECRET_KEY']
        alphabet = app.config.get('HASHIDS_ALPHABET', self._alphabet)
    ...
    def decode(self, hashid):
        ...
        try:
            ...
            ret = numbers if hashid == self.encode(*numbers) else ()
            if len(ret) == 1:
                return ret[0]
            return ret
        except ValueError:
